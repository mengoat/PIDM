...
def predict_pose(input_img):
    ...
    with torch.no_grad():
        result_img = obj.predict_pose_image("upload.jpg", num_poses=1, sample_algorithm = 'ddim',  nsteps = 50)
...
def predict_appearance(input_img):
    ...
    with torch.no_grad():
        ...
        result_img = obj.predict_appearance_image(image = "upload.jpg", ref_img = ref_img, ref_mask = ref_mask, ref_pose = ref_pose, sample_algorithm = 'ddim',  nsteps = 50)
    torch.cuda.empty_cache()  # 释放显存
    return result_img
